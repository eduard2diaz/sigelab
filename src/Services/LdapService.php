    ...
    public function search($users){
        ...
        if($info['count']==0){
            ...
            if($info['count']==1){
                ...
                $rol=$info[0]['physicaldeliveryofficename'][0];
                $facultad=$info[0]['department'][0];
                $result=['usuario'=>$usuario,'correo'=>$correo,'nombre'=>$nombre,'apellido'=>$apellidos,'facultad'=>$facultad,'rol'=>$rol];
            }else
                $result=['error'=>2];
